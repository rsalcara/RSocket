				...
				if (process) {
					...
					if (data.lidPnMappings?.length) {
						logger?.info(
							...
							'[BAILEYS-LID] 📱 Extracted LID-PN mappings from history sync'
						)
						// Log sample mappings at debug level
						if (data.lidPnMappings.length <= 5) {
							logger?.debug(
								{ mappings: data.lidPnMappings },
								'[BAILEYS-LID] 📋 LID-PN mappings detail'
							)
						} else {
							logger?.debug(
								{
						...
						logger?.debug(
							{ count: data.lidPnMappings.length },
							'[BAILEYS-LID] ✅ LID-PN mappings stored in repository'
						)
					}
