...
import { createBaileysLogger, getBaileysLogLevel } from './structured-logger'
 ...
 */
export function createDefaultPinoLogger(): Logger {
	// Dynamic import to avoid loading pino if not needed
	const pino = require('pino')

	const options: any = {
		level: getBaileysLogLevel(),
 ...
 */
let globalStructuredLogger: ReturnType<typeof createBaileysLogger> | null = null
 ...
 */
export function getStructuredLogger(baseLogger?: Logger): ReturnType<typeof createBaileysLogger> {
	if(!globalStructuredLogger) {
		const logger = baseLogger || createDefaultPinoLogger()
		globalStructuredLogger = createBaileysLogger({ baseLogger: logger })
	}
 ...
 * ```
 */
export function setStructuredLogger(baseLogger: Logger): void {
	globalStructuredLogger = createBaileysLogger({ baseLogger })
}
 ...
 */
export const legacyLoggerAdapter = {
	/**
 ...
 */
export function isStructuredLogger(logger: any): logger is ReturnType<typeof createBaileysLogger> {
