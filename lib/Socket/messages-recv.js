...
const makeMessagesRecvSocket = (config) => {
    ...
    const messageMutex = sock.messageMutex || processingMutex;
    const notificationMutex = sock.notificationMutex || processingMutex;
    const receiptMutex = sock.receiptMutex || processingMutex;
    ...
    const makeOfflineNodeProcessor = () => {
        ...
        const enqueue = (type, node) => {
            ...
            if (nodes.length >= MAX_OFFLINE_NODES) {
                ...
                logger.warn({ currentSize: nodes.length, removing: removeCount, maxSize: MAX_OFFLINE_NODES }, 'offline nodes queue exceeded limit, removing oldest entries');
    ...
    const messageHandler = (node) => {
        processNode('message', node, 'processing message', handleMessage);
    };
    const callHandler = (node) => {
        processNode('call', node, 'handling call', handleCall);
    };
    const receiptHandler = (node) => {
        processNode('receipt', node, 'handling receipt', handleReceipt);
    };
    const notificationHandler = (node) => {
        processNode('notification', node, 'handling notification', handleNotification);
    };
    const badAckHandler = (node) => {
    ...
    const connectionUpdateHandler = ({ isOnline }) => {
