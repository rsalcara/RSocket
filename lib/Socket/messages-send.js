...
const makeMessagesSocket = (config) => {
    ...
    const userDevicesCache = config.userDevicesCache ||
        new node_cache_1.default({
            ...
            maxKeys: Defaults_1.DEFAULT_CACHE_MAX_KEYS.USER_DEVICES, // 5,000 keys (memory leak prevention)
            deleteOnExpire: true,
            useClones: false
        });
    const lidCache = new node_cache_1.default({
        stdTTL: 3600, // 1 hour
        maxKeys: Defaults_1.DEFAULT_CACHE_MAX_KEYS.LID_PER_SOCKET, // 2,000 keys (memory leak prevention)
        deleteOnExpire: true,
        useClones: false
    });
