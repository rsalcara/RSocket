...
const makeMessagesSocket = (config) => {
    ...
    const userDevicesCache = config.userDevicesCache ||
        new node_cache_1.default({
            ...
            maxKeys: Defaults_1.DEFAULT_CACHE_MAX_KEYS.USER_DEVICES,
            deleteOnExpire: true,
            useClones: false
        });
    const lidCache = new node_cache_1.default({
        stdTTL: 3600, // 1 hour
        maxKeys: Defaults_1.DEFAULT_CACHE_MAX_KEYS.LID_PER_SOCKET,
        deleteOnExpire: true,
        useClones: false
    });
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        const usyncQuery = new WAUSync_1.USyncQuery().withContext('message').withDeviceProtocol();
        ...
        if (result) {
            ...
            if (lidResults.length > 0) {
                ...
                for (const item of lidResults) {
                    if (item.lid && item.id) {
                        cache_utils_1.default.lidCache.set(item.id, item.lid);
                    }
                }
            }
    ...
    const assertSessions = async (jids, force, lids) => {
        ...
        const meid = (0, WABinary_1.jidNormalizedUser)(authState.creds.me?.id);
        ...
        if (jidsRequiringFetch.length) {
            ...
            for (const jid of jidsRequiringFetch) {
                (0, Utils_1.logSession)(jid, force);
            }
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, isretry }) => {
        ...
        const lidattrs = (0, WABinary_1.jidDecode)(authState.creds.me?.lid);
        const jlidUser = lidattrs?.user;
        let lids;
        if ((0, WABinary_1.isJidUser)(jid) || (0, WABinary_1.isJidUser)(participant?.jid)) {
            const userQuery = (0, WABinary_1.jidNormalizedUser)(participant?.jid || jid);
        ...
        await authState.keys.transaction(async () => {
            ...
            if ((isGroup || isStatus) && !isRetryResend) {
                ...
                for (const { user, device, jid } of devices) {
                    const deviceServer = (0, WABinary_1.jidDecode)(jid)?.server || 'lid';
                    const senderId = (0, WABinary_1.jidEncode)(user, deviceServer, device);
                    if (!senderKeyMap[senderId] && !(0, WABinary_1.isHostedLidUser)(senderId) && !(0, WABinary_1.isHostedPnUser)(senderId) && device !== 99) {
        ...
        // Log message sent
        (0, Utils_1.logMessage)('sent', { messageId: msgId, to: jid });
