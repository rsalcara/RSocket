...
const makeChatsSocket = (config) => {
    ...
    const updateBlockStatus = async (jidOrPn, action) => {
        ...
        if ((0, WABinary_1.isLidUser)(normalizedJid)) {
            ...
            lid = normalizedJid;
            logger.debug({ lid }, '[BAILEYS-LID] 🔍 Received LID, looking up PN...');
            try {
                pn = (await signalRepository.lidMapping.getPNForLID(lid)) ?? undefined;
                if (pn) {
                    logger.debug({ lid, pn }, '[BAILEYS-LID] ✅ Found PN for LID');
                }
                else {
                    logger.warn({ lid }, '[BAILEYS-LID] ⚠️ No PN found for LID, using LID only');
                }
            }
            catch (err) {
                logger.warn({ err, lid }, '[BAILEYS-LID] ❌ Failed to lookup PN for LID');
            }
        }
        else if ((0, WABinary_1.isPnUser)(normalizedJid)) {
            // Received PN → lookup LID
            pn = normalizedJid;
            logger.debug({ pn }, '[BAILEYS-LID] 🔍 Received PN, looking up LID...');
            try {
                lid = (await signalRepository.lidMapping.getLIDForPN(pn)) ?? undefined;
                if (lid) {
                    logger.debug({ pn, lid }, '[BAILEYS-LID] ✅ Found LID for PN');
                }
                else {
                    logger.warn({ pn }, '[BAILEYS-LID] ⚠️ No LID found for PN, using PN only');
                }
            }
            catch (err) {
                logger.warn({ err, pn }, '[BAILEYS-LID] ❌ Failed to lookup LID for PN');
            }
        }
        else {
            // Unknown JID type, try to use as-is
            logger.warn({ jid: normalizedJid }, '[BAILEYS-LID] ⚠️ Unknown JID type, using as-is');
            pn = normalizedJid;
        }
        ...
        logger.info({ lid, pn, action }, `[BAILEYS-LID] 🔄 updateBlockStatus: ${action === 'block' ? '🚫 blocking' : '✅ unblocking'} user`);
        await query({
            tag: 'iq',
            attrs: {
                xmlns: 'blocklist',
        ...
        logger.info({ lid, pn, action }, `[BAILEYS-LID] ✅ Block status updated: ${action === 'block' ? 'blocked' : 'unblocked'}`);
    };
