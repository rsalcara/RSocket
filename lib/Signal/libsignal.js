    ...
    const lidMapping = {
        async getLIDPNMappings(lids) {
            ...
            if (missingLids.length > 0 && pnFromLIDUSync) {
                try {
                    ...
                    logger === null || logger === void 0 ? void 0 : logger.warn({ err, lids: missingLids }, 'Failed to fetch LID mappings via USync');
                }
            }
            return result;
        },
        async storeLIDPNMappings(mappings) {
            ...
            try {
                ...
                logger === null || logger === void 0 ? void 0 : logger.warn({ err }, 'Failed to persist LID mappings');
            }
        }
    };
    // Session migration function
    const migrateSession = async (pn, lid) => {
        try {
            ...
            if (!existingSession) {
                logger === null || logger === void 0 ? void 0 : logger.debug({ pn, lid }, 'No existing session to migrate');
                return;
            }
            ...
            logger === null || logger === void 0 ? void 0 : logger.debug({ pn, lid }, 'Session migrated successfully');
        }
        catch (err) {
            logger === null || logger === void 0 ? void 0 : logger.error({ err, pn, lid }, 'Failed to migrate session');
            throw err;
        }
    };
    return {
