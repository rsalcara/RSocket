 ...
 * ```
 */
export async function withTrace(context, fn) {
 ...
 * ```
 */
export function withTraceSync(context, fn) {
    const parentContext = traceStorage.getStore();
    const traceContext = {
        traceId: context.traceId || generateTraceId(),
        parentTraceId: parentContext?.traceId,
 ...
 * ```
 */
export function updateTraceContext(updates) {
 ...
 * ```
 */
export function createTracedLogger(baseLogger) {
    return {
        ...baseLogger,
        child: (bindings) => {
            const ctx = getTraceContext();
 ...
 */
export function isTraceContextAvailable() {
    try {
