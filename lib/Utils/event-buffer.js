...
exports.makeEventBuffer = exports.BUFFER_CONFIG = void 0;
 ...
 */
function calculateAdaptiveTimeout(metrics, config) {
    ...
    const { eventRate, averageBufferSize, averageFlushDuration } = metrics;
    const { ADAPTIVE_MIN_TIMEOUT_MS, ADAPTIVE_MAX_TIMEOUT_MS, ADAPTIVE_LEARNING_RATE, MAX_BUFFER_ITEMS } = config;
    ...
    const combinedLoad = (eventLoadFactor * 0.5 + // Event rate is most important
    ...
    metrics.mode = mode;
    // Ensure bounds
    return Math.max(ADAPTIVE_MIN_TIMEOUT_MS, Math.min(ADAPTIVE_MAX_TIMEOUT_MS, Math.floor(newTimeout)));
}
/**
 * Update adaptive flush metrics with new data
 */
function updateAdaptiveMetrics(metrics, itemsBuffered, flushDurationMs, config) {
    const now = Date.now();
...
const makeEventBuffer = (logger) => {
    ...
    const historyCacheOrder = []; // Track insertion order for LRU eviction (PR #2273)
    ...
    let adaptiveMetrics = {
        ...
        isHealthy: true,
        calculatedTimeout: exports.BUFFER_CONFIG.AUTO_FLUSH_TIMEOUT_MS,
        mode: exports.BUFFER_CONFIG.ENABLE_ADAPTIVE_FLUSH ? 'balanced' : 'disabled'
    };
    // Log adaptive flush initialization if enabled
    if (exports.BUFFER_CONFIG.ENABLE_ADAPTIVE_FLUSH) {
        logger.info({
            minTimeout: exports.BUFFER_CONFIG.ADAPTIVE_MIN_TIMEOUT_MS,
            maxTimeout: exports.BUFFER_CONFIG.ADAPTIVE_MAX_TIMEOUT_MS,
            learningRate: exports.BUFFER_CONFIG.ADAPTIVE_LEARNING_RATE
        }, 'adaptive flush enabled');
        (0, baileys_logger_1.logEventBuffer)('adaptive_flush_enabled', {
            minTimeout: exports.BUFFER_CONFIG.ADAPTIVE_MIN_TIMEOUT_MS,
            maxTimeout: exports.BUFFER_CONFIG.ADAPTIVE_MAX_TIMEOUT_MS
        });
    }
    ...
    function buffer() {
        ...
        if (buffersInProgress === 1) {
            ...
            (0, baileys_logger_1.logEventBuffer)('buffer_start');
        }
        // Start auto-flush timer when buffering begins
        if (exports.BUFFER_CONFIG.ENABLE_AUTO_FLUSH && !autoFlushTimer && buffersInProgress === 1) {
            // Use adaptive timeout if enabled, otherwise use fixed timeout
            const timeoutMs = exports.BUFFER_CONFIG.ENABLE_ADAPTIVE_FLUSH
                ? adaptiveMetrics.calculatedTimeout
                : exports.BUFFER_CONFIG.AUTO_FLUSH_TIMEOUT_MS;
            autoFlushTimer = setTimeout(() => {
                ...
                (0, baileys_logger_1.logEventBuffer)('buffer_timeout', {
                    timeoutMs,
    ...
    function flush(force = false) {
        ...
        const prometheus = (0, prometheus_metrics_1.getPrometheus)();
        if (prometheus) {
            prometheus.recordBufferFlush(adaptiveMetrics.mode, force, flushDuration, itemsBeforeFlush);
        }
        // Update adaptive flush metrics if enabled
        if (exports.BUFFER_CONFIG.ENABLE_ADAPTIVE_FLUSH) {
            updateAdaptiveMetrics(adaptiveMetrics, itemsBeforeFlush, flushDuration, exports.BUFFER_CONFIG);
            ...
            if (prometheus) {
                prometheus.updateAdaptiveMetrics(adaptiveMetrics.mode, adaptiveMetrics.calculatedTimeout, adaptiveMetrics.eventRate, adaptiveMetrics.averageBufferSize, adaptiveMetrics.isHealthy);
            }
        }
        ...
        // BAILEYS_LOG logging
        (0, baileys_logger_1.logEventBuffer)('buffer_flush', {
            flushCount: bufferMetrics.flushCount,
            historyCacheSize: historyCache.size,
            adaptiveMode: adaptiveMetrics.mode,
            flushDuration
        ...
        if (bufferMetrics.flushCount % 10 === 0) {
            ...
            (0, baileys_logger_1.logBufferMetrics)(metricsToLog);
        }
    ...
    const CLEANUP_THRESHOLD = Math.floor(exports.BUFFER_CONFIG.MAX_HISTORY_CACHE_SIZE * 0.8); // Start cleanup at 80%
    const CLEANUP_TARGET = Math.floor(exports.BUFFER_CONFIG.MAX_HISTORY_CACHE_SIZE * 0.6); // Reduce to 60% when cleaning
    function cleanHistoryCache() {
        ...
        if (historyCache.size >= CLEANUP_THRESHOLD) {
            ...
            logger.debug({
                ...
                maxSize: exports.BUFFER_CONFIG.MAX_HISTORY_CACHE_SIZE
            }, 'History cache cleanup - removing oldest entries (LRU)');
            // BAILEYS_LOG logging
            (0, baileys_logger_1.logEventBuffer)('cache_cleanup', {
                removed: removeCount,
    ...
    function destroy() {
        ...
        (0, baileys_logger_1.logEventBuffer)('buffer_destroyed', {
    ...
    return {
        ...
        emit(event, evData) {
            ...
            if (event === 'messages.upsert' && buffersInProgress) {
                ...
                if (existingUpserts.length > 0) {
                    ...
                    if (bufferedType && bufferedType !== type) {
                        ...
                        (0, baileys_logger_1.logEventBuffer)('type_mismatch_flush', {
                            bufferedType,
                            newType: type,
            ...
            if (buffersInProgress && BUFFERABLE_EVENT_SET.has(event)) {
                ...
                if (bufferMetrics.itemsBuffered > exports.BUFFER_CONFIG.MAX_BUFFER_ITEMS) {
                    // Standard logger warn
                    logger.warn({
                        itemsBuffered: bufferMetrics.itemsBuffered,
                        maxItems: exports.BUFFER_CONFIG.MAX_BUFFER_ITEMS,
                        event
                    }, 'buffer overflow detected, force flushing');
                    // BAILEYS_LOG logging
                    (0, baileys_logger_1.logEventBuffer)('buffer_overflow', {
                        itemsBuffered: bufferMetrics.itemsBuffered,
                        maxItems: exports.BUFFER_CONFIG.MAX_BUFFER_ITEMS
                    });
                    // Record Prometheus metric
                ...
                append(data, historyCache, historyCacheOrder, event, evData, logger);
                return true;
            }
...
function append(data, historyCache, historyCacheOrder, // Track insertion order for LRU eviction (PR #2273)
event, 
...
function consolidateEvents(data) {
    ...
    for (const id in data.messageReactions) {
        if (!Object.hasOwnProperty.call(data.messageReactions, id))
            continue;
    ...
    // Optimized: Direct for...in loop instead of Object.values().flatMap() (PR #2273)
    const messageReceiptList = [];
    for (const id in data.messageReceipts) {
        if (!Object.hasOwnProperty.call(data.messageReceipts, id))
            continue;
