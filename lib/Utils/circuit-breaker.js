...
class CircuitBreaker {
    ...
    recordSuccess() {
        ...
        if (this.state === 'HALF_OPEN') {
            this.successes++;
            (_a = this.config.logger) === null || _a === void 0 ? void 0 : _a.debug({ successes: this.successes, threshold: this.config.successThreshold }, 'Circuit breaker success recorded');
            // BAILEYS_LOG
    ...
    recordFailure(error) {
        ...
        if (!this.config.shouldCountError(error)) {
            (_a = this.config.logger) === null || _a === void 0 ? void 0 : _a.debug({ error: error.message }, 'Circuit breaker ignoring error (filtered out)');
            return;
        }
        const now = Date.now();
        this.failures.push({
        ...
        (_b = this.config.logger) === null || _b === void 0 ? void 0 : _b.debug({ failureCount: this.failures.length, error: error.message }, 'Circuit breaker failure recorded');
        // BAILEYS_LOG
        if ((0, baileys_logger_1.isBaileysLogEnabled)()) {
    ...
    transitionTo(newState) {
        ...
        switch (newState) {
            case 'OPEN':
                ...
                this.successes = 0;
                (_a = this.config.logger) === null || _a === void 0 ? void 0 : _a.warn({
                    oldState,
                    failures: this.failures.length,
            ...
            case 'HALF_OPEN':
                this.successes = 0;
                (_b = this.config.logger) === null || _b === void 0 ? void 0 : _b.info({ oldState }, 'Circuit breaker half-open - testing recovery');
                // BAILEYS_LOG
                if ((0, baileys_logger_1.isBaileysLogEnabled)()) {
            ...
            case 'CLOSED':
                ...
                this.successes = 0;
                this.openedAt = 0;
                (_c = this.config.logger) === null || _c === void 0 ? void 0 : _c.info({ oldState }, 'Circuit breaker closed - normal operation resumed');
                // BAILEYS_LOG
                if ((0, baileys_logger_1.isBaileysLogEnabled)()) {
