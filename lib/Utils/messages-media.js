...
async function getAudioWaveform(buffer, logger) {
    try {
        ...
        if (Buffer.isBuffer(buffer)) {
            ...
            audioData = await (0, exports.toBuffer)(rStream, 50 * 1024 * 1024); // 50MB limit for audio
        }
        else {
            // Explicit maxSize for audio streams (PR #2273 fix)
            audioData = await (0, exports.toBuffer)(buffer, 50 * 1024 * 1024); // 50MB limit for audio
        }
 ...
 */
const toBuffer = async (stream, maxSize = 100 * 1024 * 1024) => {
    const chunks = [];
    let totalSize = 0;
    try {
        for await (const chunk of stream) {
            ...
            if (totalSize > maxSize) {
                stream.destroy();
                throw new boom_1.Boom(`Stream exceeded maximum size of ${maxSize} bytes`, {
                    statusCode: 413,
...
const uploadWithNodeHttp = async ({ url, filePath, headers, timeoutMs, agent }, redirectCount = 0) => {
    ...
    const httpModule = isHttps ? await Promise.resolve().then(() => __importStar(require('https'))) : await Promise.resolve().then(() => __importStar(require('http')));
    const fileSize = (await fs_1.promises.stat(filePath)).size;
