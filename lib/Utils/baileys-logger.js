 ...
 */
export function baileysLog(message, data) {
 ...
 */
export function logConnection(event, sessionName) {
    if (!isBaileysLogEnabled())
        return;
 ...
 */
export function logDisconnect(reason, sessionName) {
    if (!isBaileysLogEnabled())
        return;
    const session = sessionName ? ` [${sessionName}]` : '';
 ...
 */
export function logQRGenerated(sessionName) {
    if (!isBaileysLogEnabled())
        return;
    const session = sessionName ? ` [${sessionName}]` : '';
 ...
 */
export function logAuth(event, sessionName) {
    if (!isBaileysLogEnabled())
        return;
    const session = sessionName ? ` [${sessionName}]` : '';
    switch (event) {
 ...
 * Log message events
 */
export function logMessage(event, details) {
    if (!isBaileysLogEnabled())
        return;
    switch (event) {
        case 'sent':
 ...
 * Log PreKey events
 */
export function logPreKeys(event, count) {
    if (!isBaileysLogEnabled())
        return;
    switch (event) {
        case 'uploaded':
 ...
 */
export function logSession(contact, forced = false) {
    if (!isBaileysLogEnabled())
        return;
 ...
 */
export function logEventBuffer(event, details) {
    if (!isBaileysLogEnabled())
        return;
    switch (event) {
        case 'buffer_start':
 ...
 */
export function logCacheMemory(event, details) {
    if (!isBaileysLogEnabled())
        return;
