 ...
 */
export function getBackoffDelay(attempt, config) {
...
export function validateRetryConfig(config) {
    ...
    if (!config.retryBackoffDelays || !Array.isArray(config.retryBackoffDelays)) {
        ...
        for (let i = 1; i < config.retryBackoffDelays.length; i++) {
            if (config.retryBackoffDelays[i] < config.retryBackoffDelays[i - 1]) {
                warnings.push(`retryBackoffDelays[${i}] (${config.retryBackoffDelays[i]}ms) is less than ` +
    ...
    if (config.maxMsgRetryCount &&
        config.retryBackoffDelays &&
        config.maxMsgRetryCount > config.retryBackoffDelays.length) {
        warnings.push(`maxMsgRetryCount (${config.maxMsgRetryCount}) exceeds retryBackoffDelays length ` +
